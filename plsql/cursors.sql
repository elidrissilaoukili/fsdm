...
DECLARE
    v_num_operation NUMBER;
    v_solde NUMBER;

BEGIN
    -- Récupérer le numéro d'opération le plus élevé dans la table BUDGET
    SELECT MAX(NUM_OPERATION) INTO v_num_operation FROM BUDGET;
    -- Incrémenter le numéro d'opération pour le nouvel enregistrement
    v_num_operation := v_num_operation + 1;
    -- Récupérer le solde correspondant au dernier enregistrement
    SELECT SOLDE INTO v_solde FROM BUDGET
    WHERE NUM_OPERATION = (SELECT MAX(NUM_OPERATION) FROM BUDGET);

    -- Insérer le nouvel enregistrement avec les valeurs spécifiées
    INSERT INTO BUDGET (NUM_OPERATION, NOM_OPERATION, CATEGORIE, DATE_OPERATION, MONTANT, SOLDE)
    VALUES (v_num_operation, 'Courses', 'Depense', TO_DATE('14/01/2002', 'DD/MM/YYYY'), 500, v_solde + 500);
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Insertion des enregistrements terminée avec succès.');
...
BEGIN
    ...
    FOR budget_record IN budget_cursor LOOP
        ...
        v_montant_euro := budget_record.MONTANT / 11;

        -- Insérer dans la table BUDGET_EURO
        INSERT INTO 
            BUDGET_EURO (NUM_OPERATION, NOM_OPERATION, CATEGORIE, DATE_OPERATION, MONTANT_EURO)
        VALUES 
            (v_num_operation, budget_record.NOM_OPERATION, budget_record.CATEGORIE, budget_record.DATE_OPERATION, v_montant_euro);

        -- Incrémenter le numéro d'opération pour le prochain enregistrement
        v_num_operation := v_num_operation + 1;
    END LOOP;
...
-- Utilisation d'un curseur paramétré :
...
DECLARE
    v_seuil NUMBER := 100; 
    -- Seuil saisi par l'utilisateur (vous pouvez le changer selon vos besoins)

    CURSOR debit_cursor(p_seuil NUMBER) IS
        SELECT NUM_OPERATION, DATE_OPERATION, MONTANT
        FROM BUDGET
        WHERE MONTANT > p_seuil AND CATEGORIE = 'Debit';
